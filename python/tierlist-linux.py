...
champion_data = pd.read_csv("COMBINED_MATCHSTATS-15.14.csv", low_memory=False).dropna(subset=["championName", "role", "win"])
...
synergy_score = np.load("synergy_score-15.14.npy")
counter_score = np.load("counter_score-15.14.npy")

model = xgb.XGBClassifier()
model.load_model("xgb_champion_model-15.14.json")
...
def save_tierlist_to_csv(tierlist, filepath="tierlist-15.14.csv"):
    rows = []
...
if __name__ == "__main__":
    random.seed(0)
    np.random.seed(0)
    tierlist = build_champion_tierlist(champion_data, threshold=69, n_samples=250000)
